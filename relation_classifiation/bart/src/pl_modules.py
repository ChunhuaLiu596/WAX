    ...
    def validation_epoch_end(self, output: dict) -> Any:
        if self.hparams.relations_file:
            ...
            if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
                ...
                scores, precision, recall, f1 = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], list(relations_cnwae.values())) 
            else:
    ...
    def test_epoch_end(self, output: dict) -> Any:
        if not self.hparams.finetune and self.hparams.relations_file:
            ...
            if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
                ...
                final_results.extend([scores["ALL"]["Macro_p"], scores["ALL"]["Macro_r"], scores["ALL"]["Macro_f1"], scores["ALL"]["f1"]])

                # with open("log/pred.csv", 'r')
                df = pd.DataFrame({"triplet_pred": [item for pred in output for item in pred['predictions']], 'triplet_true': [item for pred in output for item in pred['labels']] })
                cur_dir = os.getcwd()
                output_path = 'pred.xlsx'
                df.to_excel(output_path)
                print(f"save {cur_dir}/{output_path}")

                # 
                # dfm = df.head(131)
                # dfs = df.tail(181)
                print("predict ambiguous subset")
                scores, precision, recall, f1 = re_score([item for pred in output for item in pred['predictions']][:131], [item for pred in output for item in pred['labels']][:131],list(relations_cnwae.values()) )
                final_results.extend([scores["ALL"]["Macro_p"], scores["ALL"]["Macro_r"], scores["ALL"]["Macro_f1"], scores["ALL"]["f1"]])

                print("predict unambiguous subset")
                scores, precision, recall, f1 = re_score([item for pred in output for item in pred['predictions']][131:], [item for pred in output for item in pred['labels']][131:],list(relations_cnwae.values()) )
                final_results.extend([scores["ALL"]["Macro_p"], scores["ALL"]["Macro_r"], scores["ALL"]["Macro_f1"], scores["ALL"]["f1"]])

                final_results = pd.DataFrame([final_results], columns = ['P', 'R', 'F1', 'Acc', 'P.1', 'R.1', 'F1.1', 'Acc.1', 'P.2', 'R.2', 'F1.2', 'Acc.2'], index=[1]).round(1)

                output_path = f'results_multiple_runs.log'
                final_results.to_csv(output_path,  mode='a', header=not os.path.exists(output_path) )

                print(final_results)
                output_path = f'../../../log/results_multiple_runs.log'
                final_results.to_csv(output_path,  mode='a', header=not os.path.exists(output_path) )
