...
def extract_triplets_typed_cnwae(text, mapping_types= {'NOUN': '<noun>','VERB': '<verb>','CCONJ': '<cconj>', 'AUX': '<aux>',  'PROPN': '<propn>', 'X': '<x>', 'ADJ': '<adj>', 'DET': '<det>', 'PRON': '<pron>', 'ADV': '<adv>', 'ADP': '<adp>', 'SCONJ': '<sconj>', 'INTJ': '<intj>', 'NUM': '<num>'}):
    triplets = []
    relation = ''
    text = text.strip()
    current = 'x'
