...
def train(conf: omegaconf.DictConfig) -> None:
    ...
    tokenizer_kwargs = {
        ...
        "additional_special_tokens": ['<obj>', '<subj>', '<triplet>', '<explain>'],
    }
    ...
    if conf.dataset_name.split('/')[-1] == 'cnwae_typed.py':
        tokenizer.add_tokens(['<cconj>', '<aux>', '<verb>', '<propn>', '<x>', '<adj>', '<det>', '<pron>', '<adv>', '<adp>', '<sconj>', '<noun>', '<intj>', '<num>'], special_tokens = True)
    ...
    # df_pred = trainer.test(pl_module, test_dataloaders=pl_data_module.test_dataloader())
    # print("column_names in test: ", pl_data_module.test_dataset.column_names)
    # df_examples = []
    # for example in pl_data_module.test_dataset: 
    #     print(example)
    # #     df_examples.append((example['triplets'], example['context']))
    # # df_examples = pd.DataFrame(df_examples, columns=['triplets', 'context']) 
    # # df_examples['relation_pred'] = df_pred['relation_pred']
    # # df_examples['relation_true'] = df_pred['relation_true']

    # cur_dir = os.getcwd()
    # output_path ="{}/pred_examples.xlsx".format(cur_dir)
    # df_pred.to_excel(output_path)
    # print("Synchrosizing all predictions at {} ({} lines) ".format(output_path, len(df_examples.index)))
