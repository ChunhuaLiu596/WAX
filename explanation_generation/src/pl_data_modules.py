...
def torch_default_data_collator(features: List[InputDataClass]) -> Dict[str, Any]:
    ...
    for k, v in first.items():
        if k not in ("label", "label_ids") and v is not None and not isinstance(v, str): #220505 Posy: the isinstance(v, str) is filters some other  columns, the condition keeps only the numeratic inputs like the input_ids, labels
            # print("other k", k)
    ...
    * **test_dataloader** the test dataloader(s).
    ...
    def __init__(self, conf: DictConfig, tokenizer: AutoTokenizer, model: AutoModelForSeq2SeqLM):
        ...
        if conf.pad_to_max_length:
            ...
            self.data_collator= torch_default_data_collator
        else:
