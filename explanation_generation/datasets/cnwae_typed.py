...
class CNWAE(datasets.GeneratorBasedBuilder):
            ...
            features=datasets.Features(
                {
                    ...
                    "gold_rel": datasets.Value("string"),
                    "prompt_rel": datasets.Value("string"),
                }
            ),
    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            ...
            # datasets.SplitGenerator(name=datasets.Split.TEST, gen_kwargs={"filepath": downloaded_files["test"], "add_neg_rel": True}),
            datasets.SplitGenerator(name=datasets.Split.TEST, gen_kwargs={"filepath": downloaded_files["test"]}),
        ]


    def _generate_examples(self, filepath, add_neg_rel=False):
        ...
        if not add_neg_rel:
            with open(filepath) as json_file:
                ...
                for id_, row in enumerate(f):
                    ...
                    relation_mapped = mapping[relation['type'].lower()]
    
                    # source = ' '.join(row['tokens']) +\
                    # ' <subj> ' + ' '.join(row['tokens'][row['entities'][relation['head']]['start']:row['entities'][relation['head']]['end']]) +f' {mapping_types[row["entities"][relation["head"]]["type"]]} ' +\
                    # ' <obj> ' + ' '.join(row['tokens'][row['entities'][relation['tail']]['start']:row['entities'][relation['tail']]['end']]) + f' {mapping_types[row["entities"][relation["tail"]]["type"]]} '
                    ...
                    source = "{} and {} have a {} relation".format(subj, obj, relation_mapped)
                    target = "<subj> {} <obj> {} <explain> {}".format(subj, obj, explain)
                    print("source:", source)
                    print("target:", target)
                    ...
                    yield str(row["orig_id"]), {
                        ...
                        "prompt_rel": relation_mapped,
                        }
        else: 
            # fout = open(filepath.replace('.json', '_neg.json'), 'w')
            examples = []
            with open(filepath) as json_file:
                f = json.load(json_file)
                for id_, row in enumerate(f):
                    ...
                    for rel in mapping.values(): 
                        source = "{} and {} have a {} relation".format(subj, obj, rel)
                        target = "<subj> {} <obj> {} <explain> {}".format(subj, obj, explain)
                        print("source:", source)
                        print("target:", target)
                        # return str(row["orig_id"]), {
                        example= str(row["orig_id"]), {
                            "title": str(row["orig_id"]),
                            "context": source,
                            "id": str(row["orig_id"]),
                            "triplets": target,
